# MAX бот ЦЗН МО

Бот для мессенджера MAX
https://max.ru/murman_koncult_bot


## 🛠️ Подготовка (PROD)

Данный сервис запускается на сервере, который не имеет доступ к репозиториям docker образов, а также проблемы с получением пакетов как для python так и для node.js

#### Подготовка образа для max bot typescript

Необходимо перейти в ts_bot. Прописываем следующую команду на машине, на которой есть доступ к docker образам

linux
```bash
docker build -t max_bot_czn:v1.0 -f ./build.dockerfile .
```
Windows
```shell
docker build -t max_bot_czn:v1.0 -f .\build.dockerfile .
```

После успешной сборки, необходимо сделать архив нашего образа


Windows/Linux данную команду прописываем в директории (на флешке), так как именно там появиться архив
```bash
docker save -o max_bot_czn_v1.0.tar max_bot_czn:v1.0
```

Переходим на машину в закрытом контуре, перекидываем туда наш проект и архив с образом. Далее в директории с архивом прописываем команду

```bash
docker load -i max_bot_czn_v1.0.tar
```

Переносим бота на машину, на которую установили данный образ.
Копируем директорию ts_bot

Создаем в ней файлик .env
В котором прописан токен для работы Max бота
а также настройки прокси
```bash
BOT_TOKEN=<ваш_токен>
PROXY_USE=1
PROXY_PATH=<url_прокси>
PROXY_AUTH=0
```

если же, прокси с авторизацией то.
```bash
BOT_TOKEN=<ваш_токен>
PROXY_USE=1
PROXY_PATH=<url_прокси>
PROXY_AUTH=1
PROXY_LOGIN=<login_прокси>
PROXY_PASSWORD=<password_прокси>
```

запуск контейнера
```bash
docker-compose -f docker-compose.prod.yaml up --build
```

#### Подготовка образа для django app

Необходимо перейти в django_app. Прописываем следующую команду на машине, на которой есть доступ к docker образам

linux
```bash
docker build -t max_bot_django_app:v1.0 -f ./build.dockerfile .
```

Windows
```shell
docker build -t max_bot_django_app:v1.0 -f .\build.dockerfile .
```

После успешной сборки, необходимо сделать архив нашего образа


Windows/Linux данную команду прописываем в директории (на флешке), так как именно там появиться архив

```bash
docker save -o max_bot_django_app_v1.0.tar max_bot_django_app:v1.0
```

Переходим на машину в закрытом контуре, перекидываем туда наш проект и архив с образом. Далее в директории с архивом прописываем команду

```bash
docker load -i max_bot_django_app_v1.0.tar
```